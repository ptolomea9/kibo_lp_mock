"""
Query active keywords from Kibo Commerce Google Ads account
with impressions > 0 in the last 60 days.

This identifies keywords that are actively serving but may be missing Final URLs.
"""
...
def query_active_keywords_60d():
    """Query keywords with impressions > 0 in last 60 days from NonBrand campaigns"""
...
if __name__ == '__main__':
    print("=" * 70)
    print("Querying Active Keywords (Last 60 Days) - Kibo Commerce")
    print("=" * 70)

    print("\nQuerying keywords with impressions > 0 in last 60 days...")
    keywords = query_active_keywords_60d()

    print(f"\nFound {len(keywords)} active keywords in NonBrand campaigns")
    ...
    # Show keywords missing URLs by ad group
    print("\n" + "-" * 70)
    print("Keywords Missing URLs by Ad Group:")
    print("-" * 70)

    ad_group_missing = {}
    for kw in without_urls:
        ag = kw['ad_group_name']
        if ag not in ad_group_missing:
            ad_group_missing[ag] = []
        ad_group_missing[ag].append(kw)

    for ag, kws in sorted(ad_group_missing.items()):
        print(f"\n  {ag} ({len(kws)} keywords):")
        for kw in kws[:5]:
            print(f"    - [{kw['keyword']}] ({kw['impressions']:,} impr)")
        if len(kws) > 5:
            print(f"    ... and {len(kws) - 5} more")

    # Save to CSV
    output_file = 'C:/Users/shawh/OneDrive/Desktop/Kibo Commerce/data/active_keywords_60d.csv'
